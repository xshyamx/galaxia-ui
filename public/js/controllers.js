...
function maincontroller($scope, $http, $window, dashboardservices, $route) {
  ...
  $scope.getCount = function(val, index) {
    $http
      .get("/getEntities?unit_type=generic&sub_type=" + val)
  ...
  $scope.getDisplayData = function(option) {
    ...
    $http
      .get("/getEntities?unit_type=generic&sub_type=" + option)
      .success(function(data) {
        $scope.display = [];
        $scope.nextfield = [];
        for (i = 0; i < data.result_list.length; i++) {
          var x =
            data.result_list[i].service ||
            data.result_list[i].container ||
            data.result_list[i].node ||
            data.result_list[i].application ||
            data.result_list[i].datacenter ||
            data.result_list[i].framework;
      ...
      if (data.length == 0) {
        $http.get("/getrecmetrics/" + $scope.option).success(function(data) {
          ...
          for (i = 0; i < data[0].metrics.length; i++) {
            ...
            data[0].metrics[
              i
            ].label.instance = dashboardservices.retrieve_port();
    ...
    for (i = 0; i < metricdata.length; i++) {
      for (x = 0; x < metricdata[i].metrics.length; x++) {
        ...
        config.data.datasets.push({
          ...
          data: [],
          backgroundColor: [(bkcolor = randomColor(0.9))],
          borderColor: [bkcolor],
          borderWidth: 0,
          fill: false
        });
